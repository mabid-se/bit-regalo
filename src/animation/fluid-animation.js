...
export default class FluidAnimation {
  ...
  onMouseDown = (e) => {
    ...
    if (Math.random() < 0.33) {
      this._pointers[0].color = [0, 0.812, 0.322];
    } else if (Math.random() < 0.66) {
      this._pointers[0].color = [0.171, 0.1, 0.25];
    } else {
      this._pointers[0].color = [0, 0.223, 0.15];
    }
  };
  ...
  _initFramebuffers() {
    ...
    this._textureWidth =
      gl.drawingBufferWidth >> this._config.textureDownsample;
    this._textureHeight =
  ...
  _splat(x, y, dx, dy, color) {
    ...
    gl.uniform1f(
      this._programs.splat.uniforms.radius,
      this._config.splatRadius
    );
  ...
  update() {
    ...
    gl.uniform1i(
      this._programs.curl.uniforms.uVelocity,
      this._velocity.read[2]
    );
