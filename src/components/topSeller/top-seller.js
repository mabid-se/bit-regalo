...
export default function TopSeller(props) {
  ...
  const TopSellerDataSlice = TopSellerData.slice(0, 3);
  const TopSellerCards = TopSellerDataSlice.map((element, index) => (
    <div
      className="nft-card card seller-card shadow-sm mb-4"
      key={index}
      style={{ cursor: "pointer" }}
      onClick={() => {
        router.push(`/nft/${element.id}`);
      }}
    >
          ...
          <div className="col-8">
            <div className="name-author">
              <Link
                ...
                href={`${process.env.PUBLIC_URL}/author/${element.authorUsername}`}
              >
                {element.authorName}
                <OverlayTrigger
                  placement="top"
