...
export default function TodaysDrop(props) {
  ...
  const TodaysDropCards = TodaysDropData.slice(0, 3).map((ele, index) => (
    <div
      className="nft-card card shadow-sm mb-4"
      key={index}
      style={{ cursor: "pointer" }}
      onClick={() => {
        router.push(`/nft/${ele.id}`);
      }}
    >
        ...
        <div className="row align-items-center g-3">
          <div className="col-4">
            <div className="img-wrap">
              ...
              />
              <div
                className={`badge bg-${ele.badgeColor} position-absolute px-2 py-1`}
              >
                #{ele.id}
              </div>
            </div>
            ...
            <div className="meta-info">
              <div className="name-info d-flex align-items-center mb-3">
                <div className="author-img position-relative">
                  <img
                    className="shadow"
                    src={`${process.env.PUBLIC_URL}/${ele.authorAvater}`}
                    alt={ele.authorName}
                  />
                  <i
                    className={`bi bi-check position-absolute bg-success ${ele.authorVerified}`}
                  />
                </div>
                <div className="name-author">
                  <OverlayTrigger
                    ...
                    overlay={
                      <Tooltip id={`todaysDrop${ele.id}`}>{ele.title}</Tooltip>
                    }
                  >
                    <Link
                  ...
                  <Link
                    ...
                    href={`${process.env.PUBLIC_URL}/author/${ele.authorName}`}
                  >
                    @{ele.authorName}
                  </Link>
                </div>
              ...
              <div className="price-bid d-flex align-items-center">
                ...
                >
                  <img
                    className="me-1"
                    src={`${process.env.PUBLIC_URL}/${ele.buttonIcon}`}
                    alt="Icon"
                  />
                  {ele.buttonText}
                </Link>
  ...
  return (
    <div className="col-12 col-sm-9 col-md-6 col-lg-5 col-xl-4">
      ...
      <Link className="btn btn-minimal w-100 hover-primary" href={buttonURL}>
        {buttonText}
